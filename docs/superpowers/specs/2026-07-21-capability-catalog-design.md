...
**已知例外（实施后确认，用户裁决接受）**：目录的构造与懒加载协议（lazy）绑定，因此在 **eager 模式（`lazy_tools=false`）下技能不进 prompt** —— 此时既无目录、也不 offer `load_capabilities`，模型无从拉取技能正文。这与本节「技能无条件」的字面相悖，接受为已知取舍，理由：(1) 生产默认 `lazy_tools=true`（`config.go:234`），技能正常；(2) eager 是遗留路径，§11.1 计划最终墓碑化删除 `lazy_tools`，届时该路径不可达。若将来要在 eager 下保留技能，需一个独立设计决定（eager 也 offer `load_capabilities`，或对技能恢复正文注入），不是一行接线。
