...
**日期**：2026-08-29
**状态**：**待决策** —— 本文不是实施计划，是动手前要先敲定的边界。第 6 节列出需要拍板的三个问题。
...
**建议 fail-closed**，并把「有界」写进文档：坏插件最多让若干次调用被拒，然后自己被卸载。
...
### 决策 A：`ask`（要求人工审批）本期做不做？

- **只做 deny（A1）**：改动局限在 `internal/tool`，一周内可交付；插件能挡住危险调用，但挡不住「让人看一眼再决定」。
- **deny + ask（A2）**：要动 runtime 的挂起与检查点路径——那条路径上有过并发缺陷（`plugin/suspended` 那一轮），风险更高，工期翻倍。

**建议 A1 先行**，`ask` 单独一期。理由：`deny` 已经覆盖「插件认为这次调用不该发生」的全部刚需，而 `ask` 引入的是一条新的挂起来源，值得单独设计与验证。
...
- **进（建议）**：它是部署级事实，跨任务不变；代价是插件挂载/卸载让前缀缓存失效一次。
- 不进：每个任务都重发一遍，token 成本长期更高。
...
| **G4c 提示词段** | 稳定前缀里的命名块 + 上限 + 边界标记 | G4a | 3-4 天 |
| **G4d `ask`** | 接 `ToolGate`，新的挂起来源 | G4b；**需再评审** | 1 周+ |
