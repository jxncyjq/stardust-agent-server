...
### V3 续：内置浏览器改成安装脚本（2026-08-31，拍板：不打进安装包）

决定是**不把浏览器打进安装包**，随包带一份按平台分的安装脚本。理由是体积：每平台 150–200MB，打进去等于每次发版把它重发一遍；而版本仍然钉死——`scripts/chromium-pin.json` 里写死版本与**每个平台的 sha256**，脚本在摘要缺失或不符时**拒绝安装**（一个没被校验过、从公网下载来的浏览器，会以「App 自带组件」的身份运行）。

源用 Chrome for Testing：Google 专为自动化发布的分支，按平台分包、不会自己升级。四个平台的摘要都是实测的，其中 linux64 由 CI 打出、又在本机独立下载核对一次，**两边逐字相同**——取值路径本身先被验过。

新的风险不是下载，是**落点**：脚本装到哪、运行时去哪找，两边各写各的。错了没有任何症状——运行时找不到就静悄悄退到系统浏览器。所以查找逻辑抽进 `internal/chromium`，`cmd/wherechromium` 与 GUI 共用同一份，CI 每个平台装完之后**让运行时自己回答**它找没找到，再拿它跑一次 `--version`。

这个检查第一次跑就抓到两件事，两件都只在真机上存在：

1. **macOS 上「App 的可执行文件所在目录」不是 `build/bin`**，而是 `.app/Contents/MacOS`。给成外面那层，浏览器被装到 `.app` **旁边**，脚本一路成功、App 起来找不到。脚本现在在收到 `.app` 所在目录时直接拒绝，并把该给的路径打出来。
2. **变量名后面紧跟全角标点会炸 macOS 的 bash**：`"$version（$platform）"` 在 bash 3.2（macOS 自带）里被按字节解析成 `version（…` 这个标识符，配合 `set -u` 直接 `unbound variable` 退出；bash 5（Linux）在非 ASCII 字节处停下，所以**只有 macOS 红**。修法是一律写 `${version}`。

第二件尤其值得记：同一份脚本、同一个 shebang，在两个平台上对同一段字节的解析不同。这类差异不会出现在任何单元测试里，只会在三平台矩阵上出现。
