...
**三仓 tip**：`stardust-agent-server` `0d50c82` / `stardust-agent-gui` `6d1b763` / `docs` `8c9202d`。工作树均干净，**三仓零开放 PR**。
...
| 项 | 拍板结果（2026-09-04） |
|---|---|
| **D-1** 浏览器安装的那次询问 | **改形态：不做「首次运行弹一次」，改成放进设置菜单，用户在设置里主动安装。** 机制（`App.InstallBundledChromium()` + `chromium:install` 逐行事件）gui#42 已交付，缺的是设置面板里的入口与进度显示。这样安装永远是用户主动发起的动作——它执行的是从网上取回来的代码，主动发起比开机询问更贴合这个性质。**待做** |
