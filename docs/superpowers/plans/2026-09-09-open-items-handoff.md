...
**冷恢复已拍板：A 档（2026-09-18）**，落盘范围取「所有任务」，重启全表扫、子任务 id 用 UUID、`handleGetTaskResult` 改读表。规格见 [2026-09-18-durable-background-subtasks-design.md](../specs/2026-09-18-durable-background-subtasks-design.md)，尚无实现计划。原三档说明：已给出三档说明（2026-09-17）：A 只恢复状态（重启把 `running` 标 `interrupted`，按 id 可查，不重跑）/ B 从头重跑（工具副作用重复）/ C 断点续跑（每轮落盘整段对话，断在工具调用与结果之间的一轮无法判定是否已执行）。**推荐 A**，重跑留给调用方显式重派。附带：子任务 id 现为 `<parent>:sub-<进程内计数>`，重启后撞号，外部可寻址须改 UUID/ULID，父子关系另存字段。
