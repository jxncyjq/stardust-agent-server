...
**仍待拍板**：冷恢复的边界（恢复到哪一步算恢复）。已给出三档说明（2026-09-17）：A 只恢复状态（重启把 `running` 标 `interrupted`，按 id 可查，不重跑）/ B 从头重跑（工具副作用重复）/ C 断点续跑（每轮落盘整段对话，断在工具调用与结果之间的一轮无法判定是否已执行）。**推荐 A**，重跑留给调用方显式重派。附带：子任务 id 现为 `<parent>:sub-<进程内计数>`，重启后撞号，外部可寻址须改 UUID/ULID，父子关系另存字段。
