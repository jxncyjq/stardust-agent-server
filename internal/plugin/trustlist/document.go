...
// 内存，字节数在这一层已成定局，拦不拦都晚了。体积限制属于取回层——fetch.go
// 里的 fetchBytes 以流式方式强制**调用方传给它的那道上限**（读到上限+1 字节
// 即判定超限，在读完之前就停下）。这里限的只是 publishers 条目数：真实的
// 登记规模远小于这个数，而一份塞满条目的文档更像是有人在试探解析器，是一条
// 「这份文档不像是我签的」的判据。
