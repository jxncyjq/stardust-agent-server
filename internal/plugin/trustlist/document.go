...
// fetchBytes 以流式方式强制**调用方传进来的那道上限**（读到上限+1 字节即判定
// 超限，在读完之前就停下）。maxListBytes / maxSigBytes 是本包为清单与签名两类
// 文档各自选定的值，将由取回入口分别传给 fetchBytes；该入口尚未落地，所以这
// 两个值现在还没有生产调用方。这里限的只是 publishers 条目数：真实的登记
