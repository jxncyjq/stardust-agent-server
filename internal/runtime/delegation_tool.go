	...
	return tool.Descriptor{
		...
		InputSchema: map[string]any{
			...
			"properties": map[string]any{
				...
				"agent_id":   map[string]any{"type": "string", "description": "Optional id of a configured agent to run this sub-task as: the sub-task then runs with that agent's own tool-permission role, tool authorisation, model profile and workspace. An id that names no configured agent is refused, and so is combining it with role \"orchestrator\" — a named agent's own runtime never registers delegate_task. Combining it with toolsets is allowed: see toolsets above. Omit to run as a plain clone of this agent."},
