...
func TestRunSubTaskDepthLimitFailsLoud(t *testing.T) {
	...
	deep := NewRuntime(Config{Gate: taskgate.NewTaskGate(), Maas: maas, Role: roleOrchestrator, Depth: 2, MaxSpawnDepth: 2})
...
func TestRunSubTaskAsyncPublishesCompletion(t *testing.T) {
	...
	parent := NewRuntime(Config{Gate: taskgate.NewTaskGate(), Maas: maas, Events: events})
