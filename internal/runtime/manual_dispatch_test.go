...
func TestDispatchDeniedSensitiveReturnsRejectResult(t *testing.T) {
	...
	r := NewRuntime(Config{Gate: taskgate.NewTaskGate(), Maas: &oneToolThenTextMaas{}, Tools: reg, Checkpoints: nil, ToolGate: gate})
	...
	res, err := r.dispatchToolCall(context.Background(), domain.Agent{ID: "a1"}, task, call, &loopState{tools: reg, toolNameGuard: newSharedToolBudget()})
	if err != nil {
...
func TestDispatchDeniedLazyCallToolReturnsRejectResult(t *testing.T) {
	...
	r := NewRuntime(Config{Gate: taskgate.NewTaskGate(), Maas: &oneToolThenTextMaas{}, Tools: reg, Checkpoints: nil, ToolGate: gate, LazyTools: true})
	task := domain.Task{ID: "t1", SessionID: "s1", Mode: domain.ModeManual}
